...
# 保存训练和验证图表的函数
def save_plots(train_acc, train_loss, val_acc, val_loss, output_dir, exp_name):
    ...
    plt.title(f'Training and Validation Accuracy ({exp_name})')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.legend()
...
def load_data(dataset_file):
    ...
    df = df.dropna().reset_index(drop=True)  # 移除缺失值并重置索引
...
def main():
    ...
    # 保存图表
    save_plots(train_accuracies, train_losses, val_accuracies, val_losses, config["dump_location"], "default")
...
# 运行消融实验
def run_ablation_experiment(exp_name, tokenizer_model, model_params, config):
    ...
    model = DualBERTModel(**model_params)
    ...
    # 保存图表
    save_plots(train_accuracies, train_losses, val_accuracies, val_losses, config["dump_location"], exp_name)

    # 手动保存模型状态字典
    model_save_path = os.path.join(config["dump_location"], f'dual-bert-fake-news-model-{exp_name}')
    if not os.path.exists(model_save_path):
        os.makedirs(model_save_path)
    torch.save(model.state_dict(), os.path.join(model_save_path, 'pytorch_model.bin'))

    # 保存分词器
    tokenizer.save_pretrained(os.path.join(model_save_path, 'dual-bert-fake-news-tokenizer'))
...
if __name__ == "__main__":
    ...
    for exp_name, params in ablation_experiments.items():
        run_ablation_experiment(exp_name, params["tokenizer_model"], params["model_params"], config)
